...
function codexReview(comments, reactions, previous = {}) {
  const memory = {seen:!!previous.seen, after:{...previous.after}};
  ...
  const completed = {code:0, security:0, any:0};
  let problem = '';
  ...
  for (const summary of summaries) {
    ...
    for (const row of rows) {
      ...
      const timestamp = Date.parse(status.match(/datetime="([^"]+)"/)?.[1] || summary.updated_at) || 0;
      if (/\*\*Completed\*\*/i.test(status)) {
        ...
        problem = 'Codex review has not completed. Wait for its response; retry a failed review.';
      }
    }
  }
  ...
  if (!problem && memory.seen && !summaries.length)
  ...
  return {ok:!problem, message:problem || (summaries.length ? 'Codex review completed; findings are advisory.' : 'No active Codex review detected.'), memory};
}
...
async function run({github, context, core}) {
    ...
    for (const ref of refs) {
      ...
      for (const check of runs) {
        ...
        for (const [number,state] of Object.entries(saved)) {
          const old = reviewMemory[number] || {seen:false, after:{}};
          old.seen ||= state.seen;
  ...
  async function snapshot(open) {
    ...
    for (const p of open) {
      ...
      for (const comment of comments.filter(c => c.reactions?.eyes > 0)) {
