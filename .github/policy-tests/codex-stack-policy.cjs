...
function codexReview(comments, reactions, previous = {}) {
  const memory = {seen:!!previous.seen, after:{...previous.after}, completed:{code:0,security:0,any:0,...previous.completed}};
  ...
  memory.seen ||= !!(summaries.length || requests.length || eyes.length);
  const completed = memory.completed;
  let problem = '';
  const record = (kind, time) => { memory.after[kind] = Math.max(memory.after[kind] || 0, time || Number.MAX_SAFE_INTEGER); };
  for (const summary of summaries) {
    const rows = summary.body.split(/\r?\n/).filter(line => /^\|/.test(line) && /\*\*(?:Code Review|Security Review)\*\*/i.test(line));
    if (!rows.length) problem = 'Codex review status is unrecognized; wait for a valid completion summary.';
    for (const row of rows) {
      const kind = /\*\*Security Review\*\*/i.test(row) ? 'security' : 'code';
      const status = row.split('|')[2] || '';
      const timestamp = Date.parse(status.match(/datetime="([^"]+)"/)?.[1]) || 0;
      if (!timestamp) { problem = 'Codex review row has no valid completion/activity timestamp; waiting for a supported summary.'; continue; }
      if (/\*\*Completed\*\*/i.test(status)) {
        completed[kind] = Math.max(completed[kind], timestamp);
        completed.any = Math.max(completed.any, timestamp);
      } else {
        record(kind, timestamp);
        // Historical failed/running rows are superseded only by a later matching completion.
      }
    }
  }
  ...
  for (const reaction of eyes) record('any', Date.parse(reaction.created_at));
  if (!problem && memory.seen && !summaries.length && !completed.any)
    problem = 'Codex review activity was recorded; its completion summary is missing. Deleting a marker does not cancel the gate.';
  if (!problem && Object.entries(memory.after).some(([kind,time]) => completed[kind] <= time))
    problem = 'Waiting for a later completion of each requested or previously active Codex review.';
  return {ok:!problem, message:problem || (completed.any ? 'Codex review completed; findings are advisory.' : 'No active Codex review detected.'), memory};
}
...
async function run({github, context, core}) {
  ...
  const reviewMemory = {};
  const MEMORY_MARKER = "codex-review-memory-v2:";
  ...
  async function loadMemory(open) {
    const refs = new Set(open.map(p => p.head.sha));
    if (context.payload.before) refs.add(context.payload.before);
    for (const ref of refs) {
      const runs = await github.paginate(github.rest.checks.listForRef, {...args, ref, check_name:'Stack policy', filter:'all', per_page:100});
      for (const check of runs) {
        if (check.app?.slug !== 'github-actions' || !check.output?.text?.startsWith(MEMORY_MARKER)) continue;
        const saved = JSON.parse(check.output.text.slice(MEMORY_MARKER.length));
        for (const [number,state] of Object.entries(saved)) {
          const old = reviewMemory[number] || {seen:false, after:{}, completed:{}};
          old.completed ||= {};
          old.seen ||= state.seen;
          for (const [kind,time] of Object.entries(state.after || {})) old.after[kind] = Math.max(old.after[kind] || 0, time);
          for (const [kind,time] of Object.entries(state.completed || {})) old.completed[kind] = Math.max(old.completed[kind] || 0, time);
          reviewMemory[number] = old;
        }
      }
    }
  }
  function memoryText(prs, snapshot) {
    return MEMORY_MARKER + JSON.stringify(Object.fromEntries(prs.map(p => [p.number, snapshot?.codex[p.number]?.memory || reviewMemory[p.number] || {}])));
  }
  async function snapshot(open) {
    ...
    for (const p of open) {
      ...
      const deleted = context.eventName === 'issue_comment' && context.payload.action === 'deleted' && context.payload.issue?.number === p.number ? context.payload.comment : null;
      // Preserve a deleted command/active bot marker from the trusted event payload.
      if (deleted) comments.push(deleted);
      for (const comment of comments.filter(c => isReviewRequest(c.body) && c.user?.type === 'User')) {
        const username = comment.user.login;
        if (!permissions.has(username)) {
          const {data} = await github.rest.repos.getCollaboratorPermissionLevel({...args, username});
          permissions.set(username, ['admin','maintain','write'].includes(data.permission));
        }
        comment.authorizedRequest = permissions.get(username);
      }
      const reactions = await github.paginate(github.rest.reactions.listForIssue, {...args, issue_number:p.number, per_page:100});
      for (const comment of comments.filter(c => c.id !== deleted?.id && c.reactions?.eyes > 0)) {
        reactions.push(...await github.paginate(github.rest.reactions.listForIssueComment, {...args, comment_id:comment.id, per_page:100}));
      }
      codex[p.number] = codexReview(comments, reactions, reviewMemory[p.number]);
      reviewMemory[p.number] = codex[p.number].memory;
    }
  ...
  try {
    ...
    // Check once around the publication batch: O(N) review reads, not O(N squared).
    ...
    for (const [sha,id] of checks) {
      ...
      await github.rest.checks.update({...args,check_run_id:id,status:'completed',conclusion:bad.length?'failure':'success',
        output:{title:bad.length?'Stack policy failed':'Stack policy passed',summary,text:memoryText(prs,stable)}});
      rows.push(summary);
    }
    for (const p of stable.pulls.filter(p=>p.state==='open')) reviewMemory[p.number] = stable.codex[p.number].memory;
    const published = await snapshot(await listOpen());
    if (fingerprint(stable) !== fingerprint(published)) throw new Error('PR metadata changed during publication; revoking results.');
    ...
    for (const id of checks.values()) {
      try { await github.rest.checks.update({...args,check_run_id:id,status:'completed',conclusion:'failure',
        output:{title:'Stack validation could not complete',summary:String(error.message).slice(0,60000),text:MEMORY_MARKER+JSON.stringify(reviewMemory)}}); }
